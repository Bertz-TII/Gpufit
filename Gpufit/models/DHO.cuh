#ifndef GPUFIT_DHO_1D_CUH_INCLUDED
#define GPUFIT_DHO_1D_CUH_INCLUDED
...
__device__ void calculate_DHO_1d(
    ...
    std::size_t const user_info_size)
{
    ...
    value[point_index] = A * 2 * gamma * x0 * x0 / denominator / pi + offset;

    // derivatives
    // REAL squarred_denominator = denominator * denominator;
    REAL* current_derivatives = derivative + point_index;

    current_derivatives[0 * n_points] = 2 * gamma * x0 * x0 / denominator / pi; //d/dA

    REAL t1 = 8 * A * gamma * x0 * x0 * x0 * (x * x - x0 * x0) / (denominator * denominator) / pi;

    REAL t2 = 4 * A * gamma * x0 / pi / denominator;
                                   
    current_derivatives[1 * n_points] = t1 + t2 ;    // derivative x0

    REAL gamma1 = - 16 * A * gamma * gamma * x * x * x0 * x0 / pi / (denominator * denominator);

    REAL gamma2 = 2 * A * x0 * x0 / pi / denominator;

    current_derivatives[2 * n_points] = gamma1 + gamma2; // derivative gamma
